 ...
 *
 * PATCH: replaced std::format (C++23, no NDK 26.1 support) with
 * std::to_string (C++11) to allow compilation with NDK 26.1.
 */
...
inline folly::dynamic toDynamic(const YGValue &dimension)
{
  switch (dimension.unit) {
    ...
    case YGUnitPercent:
      // PATCH: std::format not available in NDK 26.1 — use std::to_string
